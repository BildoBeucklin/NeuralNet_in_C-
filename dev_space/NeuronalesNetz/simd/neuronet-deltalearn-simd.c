...
void testNeuralNet(struct net* net)
{
	int prediction[N_TEST_ROW] = { 0 };
	int label[N_TEST_ROW] = { 0 };
	float mse = 0;
...
#pragma omp parallel for simd
	for (int test_row = 0; test_row < N_TEST_ROW; test_row++)
	{
		for (int i = 0; i < N_OUTPUTS; i++)
		{
			...
			if (net->test_outputs[test_row][prediction[test_row]] <= net->test_outputs[test_row][i])
			{
				prediction[test_row] = i;
			}
